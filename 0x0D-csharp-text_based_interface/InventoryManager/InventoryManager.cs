...
class InventoryManager
{
    ...
    public void All(Queue<string> queue)
    {
        ...
        {
            string dequeue = queue.Dequeue();
            foreach (var item in storage.objects.Where(item => item.Key.Split(".") == dequeue))
                Console.WriteLine("{0} : {1}", item.Key, item.Value);
        }
    }

    public void Create(Queue<string> queue)
    {
        if (queue.Count > 0)
        {
            string dequeue = queue.Dequeue();
            switch (dequeue)
            {
                ...
                case "User":
                    ...
                    userName user = new User(userName);
                    storage.New(user);
                    storage.Save();
    ...
    public void Show(Queue<string> queue)
    {
        if (queue.Count > 0)
        {
    ...
    public void Delete(Queue<string> queue)
    {
        if (queue.Count > 0)
        {
            string dequeueName = queue.Dequeue();
    ...
    public void Update(Queue<string> queue)
    {
        if (queue.Count > 0)
        {
            string dequeueName = queue.Dequeue();
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            switch (dequeueName)
                            {
                                ...
                                case "User":
                                    User user = storage.objects[found];
