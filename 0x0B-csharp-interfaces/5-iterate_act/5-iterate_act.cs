...
/// <summary>
/// new class roomobjects
/// </summary>
public class RoomObjects
{
    /// <summary>
    /// iterate based on object type
    /// </summary>
    /// <param name="roomObjects"></param>
    /// <param name="type"></param>
    public static void IterateAction(List<Base> roomObjects, Type type)
    {
        foreach (var act in roomObjects)
        {
            if (type == typeof(IInteractive) && act is IInteractive)
            {
                IInteractive obj = (IInteractive)act;
                obj.Interact();
            }
            if (type == typeof(IBreakable) && act is IBreakable)
            {
                IBreakable obj = (IBreakable)act;
                obj.Break();
            }
            if (type == typeof(ICollectable) && act is ICollectable)
            {
                ICollectable obj = (ICollectable)act;
                obj.Collect();
            }
        }
    }
